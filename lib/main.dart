...
const _overlayChannel = MethodChannel('com.heartlink/overlay');
...
Future<void> showOverlay(Map<String, dynamic> drawingData) async {
  try {
    await _overlayChannel.invokeMethod('showOverlay', drawingData);
  } catch (e) {
    ...
    body: Center(child: ScaleTransition(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('HeartLink', style: GoogleFonts.playfairDisplay(
          fontSize: 36, color: const Color(0xFFF0607A),
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Center(child: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          Text('HeartLink', style: GoogleFonts.playfairDisplay(
            fontSize: 32, color: const Color(0xFFF0607A),
            fontWeight: FontWeight.bold)),
...
class _HomeShellState extends State<HomeShell> {
  ...
  Future<void> _loadUser() async {
    ...
    if (snap.exists) {
      ...
      if (_partnerId != null) _listenDrawing(_partnerId!);
    }
  }

  void _listenDrawing(String partnerId) {
    _drawSub = FirebaseDatabase.instance
      .ref('draws/done/$partnerId').onValue.listen((e) async {
      if (!e.snapshot.exists) return;
      final data = Map<String, dynamic>.from(e.snapshot.value as Map);
      await Future.delayed(const Duration(seconds: 7));
      await showOverlay(data);
    });
  }
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _loadPartnerName() async {
    final snap = await FirebaseDatabase.instance
      .ref('users/${widget.partnerId}').get();
    if (snap.exists) {
      setState(() => _partnerName =
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          const CircleAvatar(
            backgroundColor: Color(0xFFF0607A), radius: 16,
...
class _DrawScreenState extends State<DrawScreen> {
  ...
  Future<void> _sendDrawing() async {
    ...
    await FirebaseDatabase.instance
      .ref('draws/done/$_uid').set({
      'strokes': data,
      'timestamp': ServerValue.timestamp,
    });
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('💕 Drawing bheji!'),
        backgroundColor: Color(0xFFF0607A)));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF130D1A),
        title: Text('🎨 Drawing', style: GoogleFonts.playfairDisplay(
          color: const Color(0xFFF0607A))),
        actions: [
          IconButton(
          ...
          IconButton(
            icon: const Icon(Icons.send, color: Color(0xFFE8C07A)),
            onPressed: _sendDrawing,
            tooltip: 'Partner ko bhejo'),
        ],
      ),
      body: Column(children: [
        if (widget.partnerId == null)
        ...
        Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Row(children: [
              for (final c in [
                const Color(0xFFF0607A), Colors.white,
                const Color(0xFFE8C07A), Colors.lightBlueAccent,
                Colors.greenAccent, Colors.purpleAccent, Colors.black])
                GestureDetector(
    ...
    for (final s in strokes) {
      ...
      if (pts.first is Offset) {
        ...
        for (final p in pts.skip(1))
          path.lineTo((p as Offset).dx, p.dy);
      } else {
        path.moveTo((pts.first['x'] as num).toDouble(),
          (pts.first['y'] as num).toDouble());
        for (final p in pts.skip(1))
          path.lineTo((p['x'] as num).toDouble(),
            (p['y'] as num).toDouble());
      }
      ...
      child: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(
              color: const Color(0xFFF0607A).withOpacity(0.4),
