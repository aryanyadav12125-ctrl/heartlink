...
void main() async {
  ...
  await notifPlugin.initialize(
    ...
    onDidReceiveNotificationResponse: (details) {
      // Lock screen par tap hone par app khulta hai
    },
  );
...
// Lock screen drawing notification bhejo
Future<void> sendLockScreenDrawing(String partnerName) async {
  ...
  const androidDetails = AndroidNotificationDetails(
    ...
    styleInformation: BigTextStyleInformation(
      '💕 $partnerName tumhare liye draw kar raha/rahi hai!',
      summaryText: 'HeartLink',
    ),
  );

  await notifPlugin.show(
    1,
    '🎨 $partnerName Draw kar raha/rahi hai!',
    'Lock screen par dekhne ke liye tap karo 💕',
...
class _HomeShellState extends State<HomeShell> {
  ...
  Future<void> _loadUser() async {
    ...
    if (snap.exists) {
      ...
      if (_partnerId != null) _listenForDrawing(_partnerId!);
    }
  }

  void _listenForDrawing(String partnerId) {
    // Jab partner draw kare to lock screen notification bhejo
    _drawSub = FirebaseDatabase.instance
      .ref('draws/trigger/$partnerId').onValue.listen((e) async {
      if (e.snapshot.exists) {
        final snap = await FirebaseDatabase.instance.ref('users/$partnerId').get();
        final name = snap.exists
          ? (Map<String, dynamic>.from(snap.value as Map)['name'] ?? 'Partner')
          : 'Partner';
        await sendLockScreenDrawing(name);
      }
    });
  }

  @override
  void dispose() { _drawSub?.cancel(); super.dispose(); }
    ...
    final tabs = [
      _partnerId == null
        ? ConnectScreen(myCode: _myCode ?? '', onConnected: (pid) {
            ...
            _listenForDrawing(pid);
          })
        : ChatScreen(partnerId: _partnerId!),
      DrawScreen(partnerId: _partnerId, myName: _myName ?? 'User'),
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.brush_outlined), selectedIcon: Icon(Icons.brush), label: 'Draw'),
...
class _ConnectScreenState extends State<ConnectScreen> {
  ...
  Future<void> _connect() async {
    ...
    if (partnerId == uid) {
      setState(() { _msg = '❌ Apna code nahi dal sakte!'; _loading = false; }); return;
    }
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            const Text('Partner ko yeh code do', style: TextStyle(color: Colors.grey, fontSize: 12)),
          ]),
        ),
        const SizedBox(height: 24),
        TextField(
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _loadPartnerName() async {
    final snap = await FirebaseDatabase.instance.ref('users/${widget.partnerId}').get();
    if (snap.exists) {
      setState(() => _partnerName = Map<String, dynamic>.from(
        snap.value as Map)['name'] ?? 'Partner');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF130D1A),
        title: Row(children: [
          const CircleAvatar(backgroundColor: Color(0xFFF0607A), radius: 16,
            child: Icon(Icons.favorite, color: Colors.white, size: 16)),
          const SizedBox(width: 8),
          Text(_partnerName, style: GoogleFonts.playfairDisplay(
            color: const Color(0xFFF0607A))),
        ]),
      ),
...
class DrawScreen extends StatefulWidget {
  final String? partnerId;
  final String myName;
  const DrawScreen({super.key, this.partnerId, required this.myName});
  @override
...
class _DrawScreenState extends State<DrawScreen> {
  ...
  void initState() {
    ...
    if (widget.partnerId != null) {
      ...
      _sub = FirebaseDatabase.instance.ref('draws/canvas/$drawId/${widget.partnerId}')
        .onValue.listen((e) {
        if (e.snapshot.exists) {
          final data = Map<String, dynamic>.from(e.snapshot.value as Map);
  ...
  void _onPanUpdate(DragUpdateDetails d) {
    ...
    // Debounce sync
    _syncTimer?.cancel();
    _syncTimer = Timer(const Duration(milliseconds: 100), _syncDraw);
  }

  void _onPanEnd(DragEndDetails d) {
    _syncDraw();
    // Trigger lock screen notification for partner
    if (widget.partnerId != null) {
      FirebaseDatabase.instance.ref('draws/trigger/$_uid').set({
        'ts': ServerValue.timestamp, 'name': widget.myName
      });
    }
  }
  ...
  Future<void> _syncDraw() async {
    ...
    for (int i = 0; i < _myStrokes.length; i++) {
      ...
      data['s$i'] = {
        'points': pts,
        'color': _myStrokes[i]['color'],
        'size': _myStrokes[i]['size'],
      };
    }
    await FirebaseDatabase.instance.ref('draws/canvas/$drawId/$_uid').set(data);
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('🎨 Live Drawing',
          style: GoogleFonts.playfairDisplay(color: const Color(0xFFF0607A))),
        actions: [
          IconButton(
            icon: const Icon(Icons.undo, color: Colors.white),
            onPressed: () {
              if (_myStrokes.isNotEmpty) {
                setState(() => _myStrokes.removeLast());
                _syncDraw();
              }
            },
          ),
        ...
        if (widget.partnerId == null)
          Container(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(width: 8),
              Text('Partner connect karo pehle!',
...
class ProfileScreen extends StatelessWidget {
  ...
  const ProfileScreen({super.key, required this.name, required this.code,
    required this.credits, required this.onLogout});
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Padding(
      ...
      child: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              colors: [Color(0xFFF0607A), Color(0xFFE8C07A)]),
            shape: BoxShape.circle,
            boxShadow: [BoxShadow(color: const Color(0xFFF0607A).withOpacity(0.4),
        ...
        const SizedBox(height: 24),
        _card('🔑 Aapka Code', code, const Color(0xFFE8C07A), letterSpacing: 6),
        const SizedBox(height: 12),
        _card('⭐ Credits', '$credits', const Color(0xFFF0607A)),
        const SizedBox(height: 12),
        _card('🔔 Lock Screen', 'Drawing notifications ON', Colors.greenAccent),
        const Spacer(),
        ElevatedButton.icon(
          onPressed: onLogout,
  ...
  Widget _card(String title, String value, Color color, {double letterSpacing = 0}) =>
    Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white.withOpacity(0.05),
